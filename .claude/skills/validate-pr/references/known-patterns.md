...
**Frequency:** High (3 of 4 PRs tested)
**Seen in:** PR #908 (Federated Metrics — referenced `federated-metrics-operators` chart that didn't exist), PR #889 (Blob CSI — cleanup references `$CSI_BLOB_PROJECT` never created)
...
### Helm chart init containers incompatible with RHCOS
**Frequency:** Medium (ARO-specific)
**Seen in:** PR #889 (Blob CSI driver init container mounts `/usr/local` which is a symlink on RHCOS)

Helm charts designed for AKS/EKS may include init containers that write
to host paths like `/usr/local`. On RHCOS (ARO), `/usr/local` is a symlink
to `../var/usrlocal` and the container runtime cannot create the mount
target, causing `Init:CreateContainerError`.

**During execution:** When installing Helm charts on ARO, check for
hostPath volumes targeting `/usr/local`, `/opt`, or other paths that may
be symlinks or read-only on RHCOS. Disable init containers via `--set`
flags or patch the DaemonSet after install.

### Deprecated CLI flags
**Frequency:** Medium
**Seen in:** PR #889 (`az ad sp create-for-rbac --skip-assignment` is deprecated)

Azure and AWS CLI flags get deprecated over time. The command may still
work but emits warnings, or may fail in newer CLI versions.

**During execution:** Note deprecated flags as Medium-severity issues.
Remove the deprecated flag and verify the command still works.

### Globally unique resource names
**Frequency:** Medium
**Seen in:** PR #889 (storage account name `aroblobsa` already taken globally)

Azure storage accounts, Key Vaults, and some other resources require
globally unique names. Using generic names like `aroblobsa` will fail
if someone else has already claimed the name.

**During execution:** Flag generic resource names as Medium-severity.
Add a note to the guide advising users to choose unique names.

### Cleanup references non-existent resources
**Frequency:** Medium
**Seen in:** PR #889 (cleanup deletes `$CSI_BLOB_PROJECT` which was never created), PR #908

Cleanup sections may reference resources from a different version of the
guide or from an alternate deployment path that wasn't followed.

**During execution:** Cross-reference every cleanup step against the
actual resources created. Verify each resource exists before including
its deletion in the guide.
